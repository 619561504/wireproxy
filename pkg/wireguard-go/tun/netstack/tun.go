...
func (tnet *Net) NetStack() *stack.Stack {
	return tnet.stack
}
func (net *Net) DialTCPWithBind(lAddr tcpip.FullAddress, addr *net.TCPAddr) (*gonet.TCPConn, error) {
	return net.DialContextTCPWithBind(context.Background(), lAddr, addr)
}

func (net *Net) DialContextTCPWithBind(ctx context.Context, lAddr tcpip.FullAddress, addr *net.TCPAddr) (*gonet.TCPConn, error) {
	if addr == nil {
		return net.DialTCPAddrPort(netip.AddrPort{})
	}
	ip, _ := netip.AddrFromSlice(addr.IP)
	tAddr := netip.AddrPortFrom(ip, uint16(addr.Port))
	fa, pn := convertToFullAddr(tAddr)

	return gonet.DialTCPWithBind(ctx, net.stack, lAddr, fa, pn)
}

func (tnet *Net) GetAvailablePort() (int, error) {
	l, err := tnet.ListenTCP(nil)
	if err != nil {
		return 0, err
	}
	defer l.Close()
